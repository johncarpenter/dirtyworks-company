...
**No logo, font binary, or existing UI was supplied.** Icons are now licensed from IconScout and photography exists as generated candidates. See *Gaps and substitutions* below.
...
- `guidelines/*.html` — 28 specimen cards (Colors, Type, Spacing, Brand, Photography, Iconography)
- `assets/icons/registry.json` — the twenty semantic icon names, their files, fallback glyphs, meanings and IconScout provenance
- `assets/moodboard/proof-work-moodboard-v1.png` — the directional moodboard
- `assets/logo/` — exported wordmark PNGs (alpha + flattened, ink and bone), `d.` favicon at 16/32/48/180/192/512 plus an orange alt, and editable `wordmark.svg` / `favicon.svg` sources
- `assets/photography/*.png` — four generated editorial candidates
- `guidelines/photography-direction-and-image-library.md` — full photography direction, placement plan, stock shortlist, provenance register, generation prompts
...
**Photography** — four generated editorial candidates now live in `assets/photography/`, specimened in the **Photography** guideline cards; full direction, website placement plan, stock shortlist, provenance register and generation prompts are in `guidelines/photography-direction-and-image-library.md`. Direction: editorial documentary, 28–50mm, hard side light or direct flash, subjects absorbed in work rather than smiling at camera, hands marking and comparing, work surfaces with real artefacts, Alberta context via light and material. Tight crops that leave layout space; occasional monochrome halftone or single-colour duotone. Colour vibe: **warm and material**, not cool-toned tech. Banned: handshakes, boardroom lineups, holograms, glowing brains, robots, circuits, purple gradients, fake PPE staging. Generated imagery is editorial only — never captioned or positioned as a customer, employee, partner, or the founder.
...
**A real icon set is now specified.** Twenty line icons are licensed from IconScout — an evidence set (Tasks, Kiran Shastry, pack 24236) and a workflow set (Operations Management, Webtechops Llp, pack 251977) — mapped to twenty *semantic* names in `assets/icons/registry.json` and specimened in the **Iconography — the 20** card. The creative brief also explicitly directs that brand marks, diagrams, icons and layout primitives be built as vectors or code rather than raster generations, and it lists a *motif* library rather than an icon library.
...
**2. Pictogram set — licensed, activation pending.** `Icon` takes semantic names (`work-order`, `gap`, `owner`, `decision`…) and runs in two modes: `glyph` renders a Material Symbols Sharp fallback so nothing breaks while `assets/icons/` has no SVGs; `svg` renders the licensed line set, masked so `color` still applies. Flip `ICON_MODE` in `components/core/Icon.jsx` once the twenty SVGs are dropped into `assets/icons/`. Pages set `window.DS_ICON_BASE` to reach that folder. Legacy note — Where a real glyph is unavoidable (form affordances, a nav toggle, a document marker), this system links **Material Symbols Sharp** from the Google Fonts CDN via `tokens/fonts.css`, wrapped by `components/core/Icon.jsx`. It was chosen because its corners are **square** (matching the 0–3px radius system) and its weight is variable (500 matches Archivo's optical weight). Lucide/Feather/Heroicons were rejected — round line caps and 8px-radius geometry contradict the surface rules.

Rules: line only, unfilled, 20–24px, `currentColor`, weight matched to the type. Never mix in a rounded or filled set. **Never emoji.** Unicode arrows (`→ ↑`) are permitted inside diagrams as structural connectors, not as UI affordances.
...
1. **Wordmark: Route 1, decided.** The lowercase working wordmark is the chosen direction — set in type by `components/core/Wordmark.jsx` (Archivo 900, tracking −0.035em, the period before `ai` in signal orange as a registration point). Usage rules — clear space, minimum size, placement, misuse — are specimened in `guidelines/brand-wordmark.html`.
   ...
   **Still a type setting, not a drawn mark.** No logo file was supplied and none has been invented; the component renders live type, and `assets/logo/` holds PNG/SVG exports of that same type setting for places code cannot reach (favicons, social avatars, third-party portals). A licensed vector wordmark should replace it, at which point only `Wordmark.jsx` and that one card need editing. Routes 2–4 (evidence brackets / correction mark / work-order stamp) are not pursued, and no secondary seal exists.
...
3. **Icon set licensed but not yet in the repo.** The twenty IconScout SVGs were downloaded to the operator's machine; drop them into `assets/icons/` under the filenames in `registry.json`, then set `ICON_MODE = 'svg'`. Until then the Material Symbols Sharp fallback renders. **No line-art illustrations were taken** — every candidate was a corporate-blob "flying documents" scene, which the photography direction bans; service diagrams stay type-and-rule built.
4. **Photography: four generated candidates, no commissioned or licensed work yet.** `assets/photography/` holds hero/operations-leader, training-onboarding, energy-services project office, and support-incident scenes — generated originals of fictional people, usable as editorial brand imagery only. The UI kit and slides still use type, fields and diagrams; retrofitting them is open. Before public launch: commission the hero and founder photography, licence any stock selections with their source records, and choose photo-led versus type-led for the homepage hero.
